...
import { findLastX, formatDate, isDate, showFliconIcon, showIcon } from "../../../util/utils";
...
import { SiteMap, SiteMapTypeAreaGroup } from "../../../model/ModelDriven/SiteMap";
import { CustomAttribute, FetchType, SavedQuery, SavedqueryLayoutxmlGrid, savedQuery } from "../../../model/ModelDriven/SavedQuery";
import { Grid } from "../../../model/ModelDriven/Grid";
import { FormTypeTabsTabColumnSectionsSectionRowsRow, FormTypeTabsTabColumnSectionsSectionRowsRowCell, SystemForm } from "../../../model/ModelDriven/SystemForm";
...
export function parseModelDrivenScreen(modelDrivenScreen: ModelDrivenScreen) {
    ...
    const tabsCount = modelDrivenScreen.systemForm != undefined ? (Array.isArray(modelDrivenScreen.systemForm.forms.systemform.form.tabs.tab) ? modelDrivenScreen.systemForm.forms.systemform.form.tabs.tab.length : 0) : 0;

    if(tabsCount == 0) {
        addModelDrivenFrame(lastX, modelDrivenScreen, background, textColor, topColor, menucolor,tabsCount);
    } else {
        for (let index = 0; index < tabsCount; index++) {
            addModelDrivenFrame(lastX, modelDrivenScreen, background, textColor, topColor, menucolor,index);
        }
    }

}

function addModelDrivenFrame(lastX: number, modelDrivenScreen: ModelDrivenScreen, background: { r: number; g: number; b: number; } | null, textColor: { r: number; g: number; b: number; } | null, topColor: { r: number; g: number; b: number; } | null, menucolor: { r: number; g: number; b: number; } | null,tabsCount: number) {
    ...
    if (modelDrivenScreen.systemForm != undefined) {
        addFormScreen(modelDrivenScreen, background, textColor, topColor, menucolor,tabsCount);
    }
}

export function parseSavedQuery(savedQuery: SavedQuery) {
    const background = hexToRgb("#FFFFFF");
    const textColor = hexToRgb("#000000");

    const pageContent = figma.createComponent();
    pageContent.name = "SavedQuery";
    pageContent.x = findLastX();
    pageContent.y = 0;
    pageContent.fills = [{ type: 'SOLID', color: { r: Number(background?.r), g: Number(background?.g), b: Number(background?.b) } }];
    pageContent.layoutMode = 'HORIZONTAL';
    pageContent.counterAxisAlignItems = 'MIN';
    pageContent.itemSpacing = 0;

    addMainContent(pageContent,background,textColor,savedQuery);

    pageContent.layoutSizingHorizontal = "FILL";
    pageContent.layoutSizingVertical = "HUG";
}

export function parseSiteMap(sitemap: SiteMap) {
    const background = hexToRgb("#FFFFFF");
    const textColor = hexToRgb("#000000");
    const menucolor = hexToRgb("#EDEBE9");

    const pageContent = figma.createComponent();
    pageContent.name = "SiteMap";
    pageContent.x = findLastX();
    pageContent.y = 0;
    pageContent.fills = [{ type: 'SOLID', color: { r: Number(background?.r), g: Number(background?.g), b: Number(background?.b) } }];
    pageContent.layoutMode = 'HORIZONTAL';
    ...
    addSiteMapContent(pageContent,background,textColor,menucolor,sitemap);

    pageContent.layoutSizingHorizontal = "HUG";
    pageContent.layoutSizingVertical = "HUG";
}
...
function addSiteMapContent(pageContent: FrameNode | ComponentNode, background: { r: number; g: number; b: number; } | null, textColor: { r: number; g: number; b: number; } | null, menucolor: { r: number; g: number; b: number; } | null, siteMap: SiteMap) {
...
function addGroups(sitemap: FrameNode, menucolor: { r: number; g: number; b: number; } | null, textColor: { r: number; g: number; b: number; } | null, groups: SiteMapTypeAreaGroup[]) {
    for (let i = 0; i < groups.length; i++) {
        ...
        for (let index = 0; index < gr.SubArea.length; index++) {
            ...
            const groupItem = addMenuItem(menucolor,textColor,"Home",sub.Titles[0].Title["@_Title"],false,selected,true);
...
function addMainContent(pageContent: FrameNode | ComponentNode, background: { r: number; g: number; b: number; } | null, textColor: { r: number; g: number; b: number; } | null, savedQuery: SavedQuery) {
    const strokeColor = hexToRgb("#DCDBDB");
...
function addBarContent(background: { r: number; g: number; b: number; } | null, strokeColor: { r: number; g: number; b: number; } | null, textColor: { r: number; g: number; b: number; } | null,isForm: boolean) {
    ...
    bar.itemSpacing = 12;
    bar.paddingTop = 12.5;
    bar.paddingBottom = 12.5;
    bar.strokeBottomWeight = 1;

    if(isForm) {
        showFliconIcon("Left","Left", 0, 0, 20, 20, hexToRGBA("#000000", 1), bar,true);  
        showFliconIcon("Separator", "Separator", 0, 0, 22, 22, hexToRGBA("#D6D6D6", 1), bar, true);
        showFliconIcon("OpenNew", "OpenNew", 0, 0, 22, 22, hexToRGBA("#000000", 1), bar, true);
        showFliconIcon("Separator", "Separator", 0, 0, 22, 22, hexToRGBA("#D6D6D6", 1), bar, true);
...
function getName(name: string, attributes: CustomAttribute[]) {
...
function addFormScreen(modelDrivenScreen: ModelDrivenScreen, background: { r: number; g: number; b: number; } | null, textColor: { r: number; g: number; b: number; } | null, topColor: { r: number; g: number; b: number; } | null, menucolor: { r: number; g: number; b: number; } | null, tabsCount: number) {
    const strokeColor = hexToRgb("#DCDBDB");
    ...
    const headerFormName = Array.isArray(modelDrivenScreen.systemForm?.forms.systemform.form.tabs.tab) ? modelDrivenScreen.systemForm?.forms.systemform.form.tabs.tab[tabsCount]["@_name"] : modelDrivenScreen.systemForm?.forms.systemform.form.tabs.tab["@_name"];
    addHeaderForm(mainContent,background,textColor,header,headerFormName);

    if(modelDrivenScreen.systemForm != undefined) addFormContent(mainContent,modelDrivenScreen.systemForm,background,tabsCount);
...
export function addFormContent(pageContent: FrameNode | null, systemForm: SystemForm, background: { r: number; g: number; b: number; } | null, tabsCount: number) {
    const dividerColor = hexToRgb("#EDEBE9");
    ...
    const tabs = Array.isArray(systemForm?.forms.systemform.form.tabs.tab) ? systemForm?.forms.systemform.form.tabs.tab[tabsCount] : systemForm?.forms.systemform.form.tabs.tab;
    ...
    if(pageContent == null) {
        const mainContent = figma.createFrame();
        mainContent.x = findLastX();
        mainContent.y = 0;
        mainContent.name = "Main Content";
        mainContent.resize(640, 1080);
        mainContent.fills = [{ type: 'SOLID', color: { r: Number(background?.r), g: Number(background?.g), b: Number(background?.b) } }];
        mainContent.layoutMode = 'VERTICAL';
        mainContent.counterAxisAlignItems = 'MIN';
        
        mainContent.appendChild(columnFrame);
