...
import { Screen, Field, Props, Form, Container } from '../../../model/Canvas/Screen';
import {CompForm} from '../../../model/Canvas/Component';
 ...
 */
export function importYAMLFiles(files: any[]) {
    const screens : Screen[] = [];
    for (let index = 0; index < files.length; index++) {
        const file = files[index];
    
        const obj = yaml.load(file.content);
        ...
        val: for (let index = 0; index < propValues.length; index++) {
            ...
            if(fieldsKeys[index].split("As ")[1] == undefined) continue val;
            ...
            if(fieldType.includes("formViewer") || fieldType == "form") {
            ...
            if(fieldType == "group") {
                ...
                loadGroup(groupKeys,groupValues,forms,fieldName,type);
        ...
        const scrn = new Screen(screenName,0,0,screenFill,fields,forms,containers);
        screens.push(scrn);
 ...
 * gallery.
 */
function loadGallery(galKeys: string[], galValues: unknown[],forms: Form[] | CompForm[],galName: string) {
    const props: Props[] = [];
    const fields : Field[] = [];
 ...
 * form field.
 */
function loadForm(formKeys: string[], formValues: unknown[], forms: Form[] | CompForm[], fieldName: string) {
    const props: Props[] = [];
    const fields : Field[] = [];
 ...
 * group field.
 */
function loadGroup(groupKeys: string[], groupValues: unknown[], forms: Form[] | CompForm[], fieldName: string,type: string) {
    const props: Props[] = [];
    const fields : Field[] = [];
    for (let index = 0; index < groupKeys.length; index++) {
        ...
        if(key.includes("As")) {
            ...
            for (let index = 0; index < propKeys.length; index++) {
                ...
                //TODO: add validation for containers or forms under group!!!
    ...
    const group = new Form(fieldName,type,fields,props);
    forms.push(group);
}